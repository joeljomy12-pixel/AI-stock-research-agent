...
async def get_quote_fmp(symbol: str) -> Optional[QuoteData]:
    ...
    data = await _make_request(f"quote/{symbol.upper()}")
...
async def get_historical_fmp(
    ...
    data = await _make_request(f"historical-price-full/{symbol.upper()}", {"timeseries": period})
...
async def get_fundamentals_fmp(symbol: str) -> Optional[FundamentalsData]:
    ...
    profile_data = await _make_request(f"profile/{symbol.upper()}")
    if not profile_data or not isinstance(profile_data, list) or len(profile_data) == 0:
        return None
    profile = profile_data[0]

    # Get financial ratios
    ratios_data = await _make_request(f"ratios/{symbol.upper()}")
    ratios = ratios_data[0] if ratios_data and isinstance(ratios_data, list) and len(ratios_data) > 0 else {}

    # Get key metrics
    metrics_data = await _make_request(f"key-metrics/{symbol.upper()}")
    metrics = metrics_data[0] if metrics_data and isinstance(metrics_data, list) and len(metrics_data) > 0 else {}

    # Get income statement
    income_data = await _make_request(f"income-statement/{symbol.upper()}", {"period": "annual", "limit": 1})
    income = income_data[0] if income_data and isinstance(income_data, list) and len(income_data) > 0 else {}

    # Get balance sheet
    bs_data = await _make_request(f"balance-sheet-statement/{symbol.upper()}", {"period": "annual", "limit": 1})
    bs = bs_data[0] if bs_data and isinstance(bs_data, list) and len(bs_data) > 0 else {}

    # Get cash flow
    cf_data = await _make_request(f"cash-flow-statement/{symbol.upper()}", {"period": "annual", "limit": 1})
